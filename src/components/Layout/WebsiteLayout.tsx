...
const UserLayout = (props: PropsLayoutWensite) => {
    ...
    const [productsSearch, setProductsSearch] = useState<ProductTye[]>([]);
    const getCate = async () => {
        const { data } = await getAllCate();
        setCate(data)
    }
    const handlefillterCate = (e: any) => {
        props.onFillter(e.target.innerText)
    }
            ...
            <Modal title="Sản phẩm tìm được" visible={isModalVisible} width={1000} onOk={handleOk} onCancel={handleCancel}>
                <div className="container-fluid pt-5 pb-3">
                    ...
                    <div className="row px-xl-5">
                        {productsSearch.map((item) => <div className="col-lg-3 col-md-4 col-sm-6 pb-1">
                            <div className="product-item bg-light mb-4">
                                <div style={{ height: "200px" }} className="product-img position-relative overflow-hidden">
                                    <img className="img-fluid w-100" src={item.image} alt="" />
                                    <div className="product-action">
                                        <a className="btn btn-outline-dark btn-square" href=""><i className="fa fa-shopping-cart"></i></a>
                                        <Link onClick={handleCancel} className="btn btn-outline-dark btn-square" to={'/products/'+item.id}><i className="fa fa-search"></i></Link>
                                    </div>
                                </div>
                    ...
                    <div className="col-lg-4 col-6 text-left">
                        <form action="">
                            <div className="input-group">
                                ...
                                <div onClick={showModal} className="input-group-append">
                ...
                <div className="col-lg-3 d-none d-lg-block">
                    ...
                    <nav className="collapse position-absolute navbar navbar-vertical navbar-light align-items-start p-0 bg-light" id="navbar-vertical" style={{ width: "calc(100% - 30px)", zIndex: "999" }}>
                        <div className="navbar-nav w-100">
                            <div onClick={handlefillterCate} style={{ cursor: "pointer" }} className="nav-item nav-link">All</div>
                            {cate.map((item) =>
                                <div onClick={handlefillterCate} style={{ cursor: "pointer" }} className="nav-item nav-link">{item.name}</div>)}
